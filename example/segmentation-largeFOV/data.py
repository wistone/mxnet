...
class FileIter(DataIter):
    ...
    def _read(self):
        ...
        if self.shuffle:
            file_list = zip(list(data.items()), list(label.items()))
            shuffle(file_list)
            return list(zip(*file_list)[0]), list(zip(*file_list)[2])
        else:
    ...
    def _read_img(self, img_name, label_name):
        ...
        # random crop if larger than cut_off_size
        ...
        # crop if smaller than cut_off_size
        pad_h_start = pad_h_end = pad_w_start = pad_w_end = 0
        if self.cut_off_size > img.shape[0]:
            pad_h_start = round((self.cut_off_size - img.shape[0]) / 2)
            pad_h_end = self.cut_off_size - img.shape[0] - pad_h_start
        if self.cut_off_size > img.shape[1]:
            pad_w_start = round((self.cut_off_size - img.shape[1]) / 2)
            pad_w_end = self.cut_off_size - img.shape[1] - pad_w_start
        img = np.pad(img, pad_width=((pad_h_start, pad_h_end), (pad_w_start, pad_w_end), (0, 0)), mode='constant', constant_values=0)
